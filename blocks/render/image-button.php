...
$title = "";
// values from ACF
if (get_field('is_manual') && get_field('link')) {
    $bkgd   = get_field('bkgd') ? get_field('bkgd')['url'] : "";
    $link   = get_field('link') ?? '';
    $target = $link['target'] ?? '';
    $title  = $link['title'] ?? '';
    $url    = $link['url'] ?? '';
    ...
    $bkgd = "background-image: url($bkgd)";
    $target = '';
...
<a href="<?php echo $url; ?>" class="button-link image-button-block<?php echo $align_class; ?>" target="<?php echo $target; ?>" title="<?php echo $title; ?>" id="<?php echo $id; ?>" style="<?php echo isset($bkgd) ? $bkgd : ""; ?>">
